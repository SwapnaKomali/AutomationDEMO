...
class swagcartElements{
    ...
    public async addToCartElements(){
       ...
       for (let index = 0; index < sizeOfCartElements; index++) {
        ...
        async () => (await addToCartElements) == sizeOfCartElements.toString(),{timeout: 8000,timeoutMsg: " element is equal to six"})
        await expect(addToCartElements).toEqual(sizeOfCartElements.toString())
       }
    }
}
export default new swagcartElements()